            ...
            <div class="col-lg-6">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="historia_paciente.php" method="POST">
                                <div class="mb-3">
                                    <label for="selectMedico" class="form-label">Seleccione un médico</label>
                                    <select class="form-select" id="selectMedico" name="id_medico">
                                    <?php
                                    require_once("../CapaDato/medicoDAO.php");
                                    $medicoDAO = new medicoDAO($database);
                                    $medicos = $medicoDAO->obtenerTodasLasMedicos();
                                    if ($medicos !== false) {
                                        echo '<option value="">Seleccionar</option>';
                                        foreach ($medicos as $medico) {
                                            $id_medico = $medico['id_medico'];
                                            $nombreMedico = $medico['nombre'];
                                            echo '<option value="' . $id_medico . '">' . $nombreMedico . '</option>';
                                        }
                                    } else {
